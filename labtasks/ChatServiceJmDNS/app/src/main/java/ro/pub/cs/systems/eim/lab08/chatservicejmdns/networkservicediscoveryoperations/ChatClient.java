            ...
            if (printWriter != null) {
                ...
                try {
                    ...
                    while (!Thread.currentThread().isInterrupted()) {
                        String messageText = messageQueue.take();
                        if (messageText != null) {
                            Log.d(Constants.TAG, "Sending a message");
                            printWriter.println(messageText);
                            printWriter.flush();
                            Message message = new Message(messageText, Constants.MESSAGE_TYPE_SENT);
                ...
                try {
                    ...
                    while (!Thread.currentThread().isInterrupted()) {
                        ...
                        if (messageText != null) {
                            ...
                            Message message = new Message(messageText, Constants.MESSAGE_TYPE_RECEIVED);
                            conversationHistory.add(message);
                            if (context != null) {
                                ChatActivity chatActivity = (ChatActivity)context;
                                FragmentManager fragmentManager = chatActivity.getFragmentManager();
